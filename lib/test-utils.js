 ...
 * Test functions should update TestResults with the test outcome(s).
 */
...
export function checkCanvasSize(expectedWidth, expectedHeight) {
    if (canvasStatus.canvasWidth === expectedWidth && canvasStatus.canvasHeight === expectedHeight) {
...
export function checkBackground(expectedColour, colourName) {
    if (coloursMatch(expectedColour, canvasStatus.backgroundColour)) {
        TestResults.addPass(`Your sketch has the expected background colour.`);
    } else {
        const isTransparent = coloursMatch(color(...TRANSPARENT), canvasStatus.backgroundColour);
...
export function checkStrokeColour(expectedColour, colourName) {
    if (canvasStatus.hasStroke && coloursMatch(canvasStatus.strokeColour, expectedColour)) {
...
export function checkSettingCalledBeforeShapes(setting, allowedInSetup = true, allowedInDraw = true) {
    ...
    if (settingMatches.length === 0) {
        ...
        if (shapeMatches.length === 0 || shapeMatches[0].index > settingMatches[0].index) {
...
export function checkShapesWithMultipleSolutions(allSolutions, requiredBackgrounds, actual, orderMatters, colourMatters) {
    ...
    for (let s in allSolutions) {
        matchFound = orderMatters ? shapesMatchInOrder(allSolutions[s], actual, colourMatters) : shapesMatchWithoutOrder(allSolutions[s], actual, colourMatters);
        if (matchFound) {
 ...
 * @returns {boolean}
 */
function shapesMatchInOrder(expected, actual, colourMatters) {
...
function checkShapesInOrder(expected, actual, colourMatters) {
    if (shapesMatchInOrder(expected, actual, colourMatters)) {
 ...
 * @returns {boolean}
 */
function shapesMatchWithoutOrder(expected, actual, colourMatters) {
    if (expected.length !== actual.length) return false;
...
function checkShapesWithoutOrder(expected, actual, colourMatters) {
    if (shapesMatchWithoutOrder(expected, actual, colourMatters)) {
        TestResults.addPass("All expected shapes are drawn on the canvas.");
    }
    else {
        TestResults.addFail(`At least one of your shapes does not match. This could mean a shape is missing, is the wrong type of shape, is in the wrong place, has the wrong dimensions, or possibly even the wrong colour.`);
